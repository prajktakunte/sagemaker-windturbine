...
if __name__ == '__main__':
   ...
   # os.makedirs(name="/opt/ml/processing/input", mode=755, exist_ok=True)
    
    input_data_path = os.path.join("/opt/ml/processing/input", RAW_DATA_FILE)
    logger.info(f"Reading input data from {input_data_path}")

    #logger.info("Downloading data from bucket: %s ", RAW_DATA_PATH)
    #s3 = boto3.resource("s3")
    #s3.Bucket("sagemaker-us-west-2-494881137120").download_file("windturbine/data.csv", "/opt/ml/processing/input/data.csv")
